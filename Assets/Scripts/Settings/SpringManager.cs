...
namespace UnityChan
{
	public class SpringManager : MonoBehaviour
	{
		...
		[ContextMenu("Obtain bones of Kafka")]
		public void GetAllBones()
		{
			Transform[] transforms = GetComponentsInChildren<Transform>();
			foreach (Transform o in transforms)
			{
				if (o.name.StartsWith("Hair") || o.name.StartsWith("yxz__") || o.name.StartsWith("zxz_")
				|| o.name.StartsWith("ys_") || o.name.StartsWith("wt_") || o.name.StartsWith("xk_")
				|| o.name.StartsWith("zxk__") || o.name.StartsWith("yb__") || o.name.StartsWith("zb_"))

                {
					if (o.childCount > 0)
					{
						SpringBone existingSpringBone = o.GetComponent<SpringBone>();
						if (existingSpringBone != null)
						{
							DestroyImmediate(existingSpringBone);
						}
						SpringBone newSpringBone = o.gameObject.AddComponent<SpringBone>();
						newSpringBone.child = o.GetChild(0);
						newSpringBone.boneAxis = new Vector3(0, 1, 0);
					}
				}
			}
